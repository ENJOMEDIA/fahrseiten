...
/**
 * Returns a conservative PLZ-based estimate. Ambiguous border prefixes are
 * deliberately left unknown instead of presenting an inaccurate location.
 */
export function inferFederalState(postalCode: string | null) {
  const normalized = postalCode?.replace(/\D/g, "") ?? "";
  if (normalized.length !== 5) return null;
  return prefixToState[normalized.slice(0, 2)] ?? null;
}
