...
export function parseSalesCsv(source: string): SalesCsvRow[] {
  ...
  if (
    ...
    headers.some((header, index) => header !== salesCsvHeaders[index])
  )
    throw new Error(
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (cells.length !== salesCsvHeaders.length)
      throw new Error(`Zeile ${index + 2} hat nicht genau sieben Spalten.`);
    ...
    return {
      ...
      website,
      nextTaskAt: parseDate(cells[5]),
      note: z.string().trim().max(3_000).parse(cells[6]),
    };
  });
}

export function salesCsvTemplate() {
  return `${salesCsvHeaders.join(";")}\nBeispiel Fahrschule;Erika Muster;erika@example.invalid;+49 000 000000;https://example.invalid;30.09.2026 09:00;Erstkontakt vorbereiten\n`;
}
