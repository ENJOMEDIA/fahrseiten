...
Plesk zieht den Branch `main` direkt aus GitHub. Anschließend installiert das Plesk-Node.js-Toolkit mit der gewählten Node.js-22-Laufzeit die festgeschriebenen Abhängigkeiten und führt das Paket-Skript `deploy:plesk` aus. Der ausführbare Next.js-Standalone-Build entsteht unter `dist/plesk`; die Startdatei `plesk-start.mjs` im Projektstamm lädt ihn. Konfiguration, Migration, Healthcheck und Updates beschreibt die [Plesk-Deployment-Anleitung](docs/deployment-plesk.md). Die Anleitung nimmt selbst keine Hosting-, DNS- oder SSL-Änderungen vor.
