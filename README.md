...
Plesk zieht den Branch `main` direkt aus GitHub. Die zusätzliche Deployment-Aktion `bash deploy/plesk/deploy-from-git.sh` installiert die festgeschriebenen Abhängigkeiten, prüft die Plesk-Laufzeit und erzeugt unter `dist/plesk` den ausführbaren Next.js-Standalone-Build. Konfiguration, Migration, Healthcheck und Updates beschreibt die [Plesk-Deployment-Anleitung](docs/deployment-plesk.md). Die Anleitung nimmt selbst keine Hosting-, DNS- oder SSL-Änderungen vor.
