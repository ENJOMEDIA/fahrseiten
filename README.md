...
Plesk zieht den Branch `main` direkt aus GitHub. Anschließend installiert das Plesk-Node.js-Toolkit über **Pakete installieren** mit der gewählten Node.js-22-Laufzeit die festgeschriebenen Abhängigkeiten. Danach führt das Paket-Skript `deploy:plesk` die Prüfungen und den Build ohne einen verschachtelten zweiten pnpm-Aufruf aus. Der ausführbare Next.js-Standalone-Build entsteht unter `dist/plesk`; die Startdatei `plesk-start.mjs` im Projektstamm lädt ihn. Konfiguration, Migration, Healthcheck und Updates beschreibt die [Plesk-Deployment-Anleitung](docs/deployment-plesk.md). Die Anleitung nimmt selbst keine Hosting-, DNS- oder SSL-Änderungen vor.

Die konkrete Reihenfolge für netcup/Plesk vom Git-Pull über Datenbank, DNS und SSL bis zu Installer, SMTP, Scheduler und späteren Updates steht in der [Livegang-Checkliste](docs/go-live-plesk.md). Der erzeugte Standalone-Build enthält zusätzlich `GO-LIVE.md`, die SQL-Fallbackdatei und eine geheimnisfreie Variablenvorlage. Der GitHub-Workflow `Plesk-Artefakt` bleibt ein optionaler, geprüfter Fallback und ist für das direkte Plesk-Deployment nicht erforderlich.

Das vollständige SQL-Schema entsteht mit `pnpm db:schema:bundle` aus allen versionierten Migrationen. Nach dem ersten Git-Deployment fragt `/setup` die Zugangsdaten einer zuvor in Plesk angelegten leeren Datenbank ab und führt durch deren einmalige Initialisierung, den ersten Plattform-Owner und die FahrSeiten-Stammdaten. Die Datenbankverbindung wird mit restriktiven Dateirechten außerhalb des Git- und Application-Root gespeichert. `node install.mjs` bleibt als Shell-Alternative verfügbar.

Gebuchte Fahrschulen erhalten keine eigene Anwendung und keine eigene Datenbank. Ein Plattform-Owner erzeugt unter `/admin/mandanten/neu` einen 14 Tage gültigen Einmal-Link. Darüber legt die Fahrschule ihre Stammdaten, Inhaber- und Zugangsdaten, gewünschte Domain sowie Primär- und Akzentfarbe fest. Der Assistent erzeugt den neuen Mandanten atomar in der gemeinsamen Datenbank; die Domain bleibt bis zur separaten DNS- und SSL-Prüfung im Status `pending`.
